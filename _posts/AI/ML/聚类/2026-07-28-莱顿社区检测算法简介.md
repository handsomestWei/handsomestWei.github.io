...
业务落地时，Leiden 只负责「切社群」；**图从哪来**往往更决定效果。常见流水线如下：

```text
① 基于业务规则定义建边规则
   · 强关联、可区分的字段可直接建边
     （如：同设备、同手机号、同银行卡、同收货地址指纹…）
   · 过宽泛的字段不宜单独建边
     （如：籍贯、同城、同姓）——否则图会过度连通、社群糊成一团
   · 宽泛字段可作边权加成 / 加权因子
     （已有强边时，同籍贯可提高权重，但不单独拉边）
        ↓
② 基于边规则构建图
   · 节点：用户 / 账号 / 商户等实体
   · 边：满足规则则连边；权重 = 规则强度 + 可选加成
   · 产出：igraph / networkx 等图对象（可加权）
        ↓
③ 使用 Leiden 算法切分社群
   · 选择质量函数（Modularity / CPM 等）与分辨率 γ
   · 输出 community 标签 → 团伙名单 / 运营分群 / 下游特征
```

| 环节 | 要点 |
|------|------|
| **建边规则** | 先定「什么叫有关系」；宁缺毋滥，避免宽泛属性直接成边 |
| **权重** | 强规则定有无；籍贯等弱信号只做加成，防止假连接 |
| **Leiden** | 在已建好的图上做社区发现；图质量差时调算法也难救 |

一句话：**规则建边 → 构图 → Leiden 切群**；算法再强，也补不回建边过宽带来的噪声连通。
